...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if SoundDescriptor.objects.filter(descriptor=descriptor).exists():
            self.stdout.write(self.style.WARNING(f'Sound descriptor "{descriptor}" already exists.'))
        else:
            # Create and save the new descriptor
            SoundDescriptor.objects.create(descriptor=descriptor)
            self.stdout.write(self.style.SUCCESS(f'Successfully added descriptor "{descriptor}".'))
