...
class Command(BaseCommand):
    ...
    def get_sound_from_descriptor(self, client, descriptor):
        try:
            ...
            results = client.text_search(query=descriptor, fields="id,name,previews,duration")
            ...
            if results.count > 0:
                ...
                self.stdout.write(self.style.WARNING(f"No sounds found for {descriptor}"))
                return None
    ...
    def upload_sound_to_s3(self, client, sound_url, unique_key):
        ...
        try:
            ...
            self.stdout.write(self.style.ERROR(f"Error uploading sound to S3: {str(e)}"))
            return None
